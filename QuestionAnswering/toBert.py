...
img2comment = {}
train_file = open("./Data/SemArt/semart_train.csv", encoding='mac_roman')
...
val_file = open("./Data/SemArt/semart_val.csv", encoding='mac_roman')
...
for row in csv_reader:
    if line_count == 0:
        ...
        line_count += 1
print(f'Processed {line_count} lines.')
test_file = open("./Data/SemArt/semart_test.csv", encoding='mac_roman')
...
with open('./Data/Bert/comment_prediction_train.json') as json_file:
...
with open('./Data/Bert/comment_prediction_val.json') as json_file:
    content = json.load(json_file)
    for example in content:
        question = example['question']
        positive = example['image']
...
with open('./Data/Bert/comment_prediction_test.json') as json_file:
    content = json.load(json_file)
    for example in content:
        question = example['question']
...
train_df.to_csv('./Data/Bert/qaFromComments_train.csv', index=False)
val_df = pd.DataFrame(data=val_data, columns=col)
val_df.to_csv('./Data/Bert/qaFromComments_val.csv', index=False)
test_df = pd.DataFrame(data=test_data, columns=col)
test_df.to_csv('./Data/Bert/qaFromComments_test.csv', index=False)
